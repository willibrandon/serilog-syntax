#!/bin/bash
# Quick deploy script for SerilogSyntax (WSL/Linux version)
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: $0 [-c|--configuration Debug|Release] [--no-build] [--restart-vs] [--experimental] [--all]"
      exit 1
      ;;
  esac
done
...
PS_CMD="powershell -ExecutionPolicy Bypass -File scripts\\quick-deploy.ps1 -Configuration $CONFIGURATION"
